...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, Serializable {
    ...
    static final <K, V> Node<K, V> tabAt(Node<K, V>[] tab, int i) {
        ...
        return (Node<K, V>) U.getObjectVolatile(tab, offset);
    }
    ...
    static final <K, V> boolean casTabAt(Node<K, V>[] tab, int i, Node<K, V> c, Node<K, V> v) {
        // 计算下标i的元素在底层的偏移地址
        long offset = ((long) i << ASHIFT) + ABASE;
        return U.compareAndSwapObject(tab, offset, c, v);
    }

    // 原子的设置tab[i]的节点为node
    static final <K, V> void setTabAt(Node<K, V>[] tab, int i, Node<K, V> v) {
        long offset = ((long) i << ASHIFT) + ABASE;
        U.putObjectVolatile(tab, offset, v);
    }
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        // 若key为null或value为null 直接抛出空指针移除
        ...
        for (Node<K, V>[] tab = table; ; ) {
             ...
             */
                ...
                // 需要将节点添加hash表中f位置对应的链表或红黑树中
                V oldVal = null;
                // 锁定f节点
                synchronized (f) {
                    // 若tab[i]==f，则代表当前待插入状态仍然可信
                    if (tabAt(tab, i) == f) {
                        // 如果当前哈希槽首个元素是普通结点
                        if (fh >= 0) {
                            binCount = 1;
    ...
    private final Node<K, V>[] initTable() {
        ...
        while ((tab = table) == null || tab.length == 0) {
            sc = sizeCtl;
            // 当sc小于0是表示正在初始化或正在扩容，则当前线程让出CPU时间片
            if (sc < 0) {
                Thread.yield(); // lost initialization race; just spin
            }
            // CAS原子地将sizeCtl字段更新为-1，代表当前Map进入初始化阶段
    ...
    private final void treeifyBin(Node<K, V>[] tab, int index) {
        ...
        int n, sc;
        if (tab != null) {
            // 若当前哈希数组的数据容量小于64，则容量还未达到形成一棵红黑树的最低要求
            if ((n = tab.length) < MIN_TREEIFY_CAPACITY) {
                // 尝试调整哈希数组的大小，以容纳指定数量的元素（默认增加一倍）
                tryPresize(n << 1);
            }
            // 哈希数组的数据容量达到形成一棵红黑树的最低要求,则将链表转换为红黑树
            else {
                // 并且该位置的节点不为空，并且b.hash不小于0,否则
                b = tabAt(tab, index);
                if (b == null || b.hash < 0) {
                    return;
                }
                ...
                synchronized (b) {
                    ...
                    if (tabAt(tab, index) == b) {
                        ...
                        TreeNode<K, V> hd = null; // 保存节点的头部
                        TreeNode<K, V> tl = null;
                        // 1.循环将所有普通结点“转换”为红黑树结点并串联起来
