 ...
 *
 * HashMap的key值必须重写hashcode()和equals()方法
 *
...
public class HashMap<K, V> extends AbstractMap<K, V>
     ...
     * shrinkage.
     */
    // hash冲突的链表进行树化的阈值，当大于等于阈值时转换为二叉树
     ...
     */
    // 当hash冲突中的二叉树转换为链表的阈值，当小于等于阈值时转换为链表
    ...
    /**
     * 此哈希映射被结构修改的次数，
     * 结构修改是指更改映射数的哈希映射或以其他方式修改其内部结构（例如，重新哈希）。
     * 此字段用于在集合视图上迭代哈希映射快速失败。 （请参阅并发修改异常）。
     */
    ...
    final void treeifyBin(Node<K, V>[] tab, int hash) {
        int n, index;
        Node<K, V> e;
        // 若哈希数组没有初始化，或哈希数组的容量还未达到形成一棵红黑树的最低要求
    ...
    // 获取指定的key的value值，若该key在map中不存在，则返回默认值
    @Override
    ...
    // 替换拥有指定key和value的元素的value为newValue值，返回值表示是否替换成功
    @Override
